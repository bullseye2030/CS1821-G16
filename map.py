...
class Map:
    ...
    def __init__(self, items, grid_start_coords, grid_dims, rows, columns, obstacle_rate, obstacle_min_size=2,
                 obstacle_max_size=4, debug=False):
        self.items = items  # should be changed to just instantiate walls here
        self.grid_start_coords = grid_start_coords  # pixel coords of the top left corner of the grid where 0,0 would be
        ...
        self.image = simplegui.load_image(
            ...
            '/X8MWUElYqJ5DFjHbuOF2diN9ARFFPVqp3CHJNx8XCItvgxNKeNMIOn_iEmv2Bkqk88Qx9hw1bQmFSWDF1BOX=s400')
        ...
        for y in range(self.rows):  # y because go down columns first then pick a row number
            for x in range(self.columns):
                if gamemap[y][x] != 0:  # check if map already has an obstacle there; if it is 1 then ignore
                    # pick direction of growth (pick whether one arm of the obstacle will go left or right)
                    direction_x = -1 if randint(0, 1) % 0 == 0 else 1
            ...
            for y in range(self.columns):
                if randint(1, self.obstacle_rate) == 1:  # a 1/obstacle rate chance of creating an obstacles
                    ...
                    if obstacle == 1:
                        ...
                        temp_map = self.build_block(temp_map, start_coords, size)  # size of generated obstacles
