...
class Wall:
    ...
    def check_collision(self, item) -> bool:
        if self.face == 'l':
            hit = (item.pos.x <= self.edge)
        elif self.face == "r":
            hit = (item.pos.x >= self.edge)
        elif self.face == 't':
            hit = (item.pos.y <= self.edge)
        elif self.face == "b":
            hit = (item.pos.y >= self.edge)
        else:
            raise Exception("Invalid direction")
        if hit:
            item.collide_wall(self)
...
class Map:
    ...
    def __init__(self, grid_start_coords, grid_dims, rows, columns, obstacle_rate, obstacle_min_size=2,
                 obstacle_max_size=4, mirror_map=True, debug=False):
        self.items = []  # should be changed to just instantiate walls here
        ...
        self.heatmap = self.generate_heat_map()
        if self.mirror_map:
            self.heatmap = [mirror_list(row) for row in self.heatmap]
        self.obstacles, self.floor_tiles = self.heatmap_to_obstacles(self.heatmap)
    ...
    def heatmap_to_obstacles(self, heatmap):
        ...
        for x in range(0, self.rows):
            for y in range(0, self.columns):
                if heatmap[x][y] == 1:
                    new_y = self.obs_dims[1] * y + self.grid_start_coords[1]
                    new_x = self.obs_dims[0] * x + self.grid_start_coords[0]
    ...
    def gen_spawn_t1(self):
        """
        for team 1, the player team
        generates a new spawn location by checking the heatmap and other tank spawns for a free space
        works using the grid system (so not pixels) right up until the end where it returns coords in pixelspace
        ...
        while not valid_spawn:
            ...
            for tank_spawn in self.t2_spawns:
                if not self.check_spawn_dist((spawn_x, spawn_y), tank_spawn, enemy_min_distance):
                    continue
            valid_spawn = True
        new_spawn = (spawn_x, spawn_y)      #store spawn location in list of existing spawns
        self.t1_spawns.append(new_spawn)
        new_y = self.obs_dims[1] * spawn_y + self.grid_start_coords[1]
        new_x = self.obs_dims[0] * spawn_x + self.grid_start_coords[0]
        return (new_x, new_y)

    def gen_spawn_t2(self):
        """
        for team 2, the enemy team
        generates a new spawn location by checking the heatmap and other tank spawns for a free space
        works using the grid system (so not pixels) right up until the end where it returns coords in pixelspace
        :return: coordinates (pixels)
        """
        ...
        while not valid_spawn:
            ...
            valid_spawn = True
        new_spawn = (spawn_x, spawn_y)      #store spawn location in list of existing spawns
        self.t2_spawns.append(new_spawn)
        new_y = self.obs_dims[1] * spawn_y + self.grid_start_coords[1]
        new_x = self.obs_dims[0] * spawn_x + self.grid_start_coords[0]
        return (new_x, new_y)
